...
export const Product = () => {
  ...
  const cartHolderHandlerMainAdd = (p) => {
    //handling the main product (into cart) ADDING AMOUNT
  ...
  const cartHolderHandlerMainMin = (p) => {
    //handling the main product (into cart) DECREASING AMOUNT
    let cont = cartHolder[0].amount;
    if (cartHolder[0].amount > 1) {
      setCartHolder(
        (cartHolder[0] = [
  ...
  const cartHolderRecoAdd = (evt, p) => {
    //INCREASING AMOUNT CART HOLDER RECO PROD
